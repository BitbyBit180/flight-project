...
export class Renderer {
  ...
  private drawOverlays(cfg: Config, proj: ProjOpts): void {
    ...
    if (cfg.rangeRings) {
      ctx.save();
      const ringStep = cfg.radiusKm > 100 ? 50 : cfg.radiusKm > 30 ? 10 : 5;
      for (let km = ringStep; km <= Math.floor(cfg.radiusKm); km += ringStep) {
  ...
  private labelLines(cfg: Config, ac: Aircraft): { text: string; kind: "title" | "sub" }[] {
    ...
    if ((f.origin || f.destination) && routePlausible(ac, cfg)) {
      const hasOrigin = f.origin && !!ac.origin;
      const hasDest = f.destination && !!ac.destination;
      if (hasOrigin || hasDest) {
        let head: string;
        if (hasOrigin && hasDest) head = `${ac.origin} → ${ac.destination}`;
        else if (hasOrigin) head = `${ac.origin} →`;
        else head = `→ ${ac.destination}`;
        if (hasDest && ac.destName) head = `${head}   ${ac.destName}`;
        out.push({ text: head, kind: "sub" });
        if (hasDest && cfg.showRouteDetail && ac.destLat != null && ac.destLon != null) {
  ...
  private drawDetailPanelText(cfg: Config, v: Visible, ac: Aircraft, x: number, y: number): void {
    ...
    const bits = [
      ...
      (() => {
        if (!routePlausible(ac, cfg)) return null;
        const hasO = cfg.showFields.origin && !!ac.origin;
        const hasD = cfg.showFields.destination && !!ac.destination;
        if (hasO && hasD) return `${ac.origin} → ${ac.destination}`;
        if (hasO) return `${ac.origin} →`;
        if (hasD) return `→ ${ac.destination}`;
        return null;
      })(),
    ].filter(Boolean);
