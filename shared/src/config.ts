...
export interface Config {
  ...
  centerLat: number;
  centerLon: number;
  /** How far out to show, in **kilometers**. Default 3 — "what you could
   *  realistically see" from a single point. The Pi/UI cap is 500 km;
   *  the server caps the API radius at 500 nautical miles. */
  radiusKm: number;
...
export function mergeConfig(base: Config, patch: Partial<Config>): Config {
  // One-time migration: old persisted configs used `radiusMiles`. Convert
  // to `radiusKm` (1 mi = 1.60934 km) and drop the legacy key.
  const legacyMiles = (patch as { radiusMiles?: number }).radiusMiles;
  const migrated: Partial<Config> = { ...patch };
  if (legacyMiles != null && migrated.radiusKm == null) {
    migrated.radiusKm = Math.round(legacyMiles * 1.60934 * 10) / 10;
  }
  delete (migrated as { radiusMiles?: number }).radiusMiles;
  return {
    ...base,
    ...migrated,
    palette: { ...base.palette, ...(migrated.palette ?? {}) },
    fonts: { ...base.fonts, ...(migrated.fonts ?? {}) },
    showFields: { ...base.showFields, ...(migrated.showFields ?? {}) },
  };
}
