...
export class Poller {
  ...
  private buildApiUrl(): string {
    ...
    // km → nautical miles for the airplanes.live `r` param. Cap at 500 NM
    // (~926 km) — most ADS-B aggregators don't return anything useful
    // beyond that anyway.
    const nm = c.radiusKm / KM_PER_NM;
    const r = Math.min(500, Math.max(1, Math.ceil(nm) + 1));
